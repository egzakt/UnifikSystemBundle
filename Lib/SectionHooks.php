...
class SectionHooks
{
     ...
     * return array(
     *     12 => array(
     *         'repository' => 'UnifikBackendCareerBundle:Category',
     *         'method' => 'findAll',
    ...
    public function findSectionInTree($sectionId, $elements)
    {
        ...
        foreach ($elements as $element) {
            ...
            if ($element->getEntity()->getId() == $sectionId && ($element->getEntity() instanceof Section || strstr(get_class($element->getEntity()), 'UnifikBackendSectionBundleEntitySectionProxy'))) {
                // Return this element
                return $element;
            }
